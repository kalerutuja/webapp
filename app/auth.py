...
# aws_access_key = 'AKIASMC4BZS6INDZZN6W'
# aws_secret_key = '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'
...
@auth.route('/sign-up', methods=['GET','PUT'])
def signup():
    msg = "welcome"
    db.create_all()
    db.session.commit()
...
@auth.route('/pic',methods=['GET','POST','DELETE'])
@token_required
def pic(curr_user):
    msg = "welcome"
    result = []
        ...
        if user:
            ...
            if not file.startswith('~'):
                try:
                    ...
                    user_info["Image Name"] = file
                    user_info["Upload Date"] =date.today()
                    user_info["url"] ="https://s3.amazonaws.com/"+bucket_name+"/"+user.uname+"/"+file
                    result.append(user_info)
                    head_object = client_s3.head_object(Bucket=bucket_name,Key=object_name)
                    new_user = Pic(fname=user.fname, lname=user.lname, uname=user.uname, profile=file)
                    db.create_all()
                    db.session.commit()
                    db.session.add(new_user)
                    db.session.commit()
                    result.append(head_object)
                    msg = make_response(jsonify({"success": "User's profile picture successfully added!", "User Information": result}), 201)
    ...
    if request.method == 'GET':
        if user:
            ...
            for obj in bucket.objects.filter(Prefix=user.uname+'/'):
                ...
                msg = make_response(jsonify({"success": "User's profile picture found!", "Image Name": result}), 201)
        else:
            msg = make_response(jsonify({'error': "User doesn't exist!"}), 404)
